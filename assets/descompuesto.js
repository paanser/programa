...
// ══════════════════════════════════════════════════════════
//  RENDER PRINCIPAL — llamar desde cada página adaptadora
// ══════════════════════════════════════════════════════════

function mm(v) { return Math.round(v); }

/**
 * Renderiza el descompuesto S28 en el elemento contenedor dado.
 *
 * @param {HTMLElement} containerEl  Elemento donde se vuelca el HTML
 * @param {string}      systemKey    Clave de SYSTEMS (p.ej. 'v2h_prac')
 * @param {number}      L            Ancho del hueco en mm
 * @param {number}      H            Alto del hueco en mm
 * @param {number}      qty          Número de unidades
 * @param {object}      opts         { glassThick, junquilloType, forroType, premarcoType }
 */
function renderS28Results(containerEl, systemKey, L, H, qty, opts) {
    const glassThick   = opts.glassThick   || 20;
    const junqType     = opts.junquilloType || 'curvo_clip';
    const forroType    = opts.forroType    || 'none';
    const premarcoType = opts.premarcoType || 'none';

    const sys = SYSTEMS[systemKey];
    if (!sys || L < 100 || H < 100) {
        containerEl.innerHTML =
            '<p class="field-hint" style="padding:1rem">Introduce dimensiones válidas (mín. 100 mm).</p>';
        return;
    }

    const result = sys.calc(L, H, qty, { glassThick, junquilloType: junqType });
